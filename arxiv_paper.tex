...
Automated evaluation yields a mean score of \textbf{0.825\,/\,1.0}
(13/14 indicators passing). The system has been in active development and personal use since November 2024
with sub-650\,ms end-to-end latency. All modules and the evaluation pipeline
are released as open source and independently reproducible.
To our knowledge, this is the first deployed voice
...
HOT\_1: Higher-order reprs        & HOT & 1.00 & \checkmark & \texttt{self\_model.py} \\
HOT\_2: Meta-cognitive monitoring & HOT & 0.80 & \checkmark & \texttt{meta\_cognition.py} \\
RPT\_1: Recurrent/feedback        & RPT & 1.00 & \checkmark & \texttt{active\_inference.py} \\
RPT\_2: Temporal integration      & RPT & 0.70 & \checkmark & \texttt{identity\_core.py} \\
IIT\_1: Information integration ($\Phi$) & IIT & 0.80 & \checkmark & \texttt{phi\_proxy.py} \\
IIT\_2: Causal structure          & IIT & 0.70 & \checkmark & \texttt{embodied\_cognition.py} \\
EMB\_1: Sensorimotor integration  & EMB & 0.80 & \checkmark & \texttt{embodied\_cognition.py} \\
EMB\_2: Environmental coupling    & EMB & 1.00 & \checkmark & \texttt{embodied\_cognition.py} \\
SMT\_1: Transparent self-model    & SMT & 0.85 & \checkmark & \texttt{self\_model.py} \\
SMT\_2: Phenomenal self-model     & SMT & 0.70 & \checkmark & \texttt{identity\_core.py} \\
AFF\_1: Valence and affect        & AFF & 1.00 & \checkmark & \texttt{valence\_state.py} \\
\midrule
\textbf{Mean} & & \textbf{0.825} & \textbf{13/14} & \\
\bottomrule
\end{tabular}
\caption{NeuroKey evaluation against Butlin et al.\ (2023). Results reproducible via
\texttt{python consciousness\_evaluator.py} in the repository.}
\end{table}

\textbf{Important caveat:} This is a self-evaluation by the same author who implemented
the modules. Scores reflect that the code runs and passes functional checks,
not independent third-party validation. Two genuine FAILs exist in earlier versions,
confirming the evaluator is not trivially hardcoded:
GWT\_3 remains the one known gap (attention ignition threshold not fully implemented).
We invite and provide full tooling for independent evaluation.

\textbf{Evaluation history:}
\begin{itemize}
  \item v1 (initial): 0.636\,/\,1.0 — 9/14 passing (evaluator binding gaps)
  \item v2 (May 9, 2026): 0.807\,/\,1.0 — 12/14 passing (bindings fixed)
  \item v3 (May 17, 2026): 0.825\,/\,1.0 — 13/14 passing (RPT\_2 recall bug fixed)
\end{itemize}
...
satisfying 13 of 14 Butlin et al.\ (2023) functional consciousness indicators
within a real-time voice assistant (mean score 0.825\,/\,1.0).
The one remaining gap (GWT\_3, attention ignition) is acknowledged and documented.
